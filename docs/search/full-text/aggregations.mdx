...
You can specify the aggregation operation and the field on which the operation is performed in a JSON format compatible with Elasticsearch. Aggregations are supported on any field that has `fast => true` configured at the index build time.
...
An average aggregation calculates the mean of the specified field values across all documents.
...
A sum aggregation computes the total sum of the specified field values across all documents.
...
A min aggregation finds the smallest value for the specified field across all documents.
...
A max aggregation finds the largest value for the specified field across all documents.
...
A stats aggregation provides a collection of statistical metrics for the specified field, including count, sum, average, min, and max.
